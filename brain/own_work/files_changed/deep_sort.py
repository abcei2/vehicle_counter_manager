...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, classes, ori_img, use_yolo_preds=True):
        ...
        for track in self.tracker.tracks:
            ...
            if not track.is_confirmed() or track.time_since_update >= track._max_age-2:
                is_lost = True
                outputs.append(np.array([0, 0, 0, 0, track.track_id, track.class_id, is_lost], dtype=np.int))
                continue
            if use_yolo_preds:
            ...
            outputs.append(np.array([x1, y1, x2, y2, track_id, class_id, is_lost], dtype=np.int))
        if len(outputs) > 0:
